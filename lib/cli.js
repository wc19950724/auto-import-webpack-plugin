...
const projectIgnorePath = path$5.resolve(projectPath, options.ignorePath);
const ig = ignore$1();
...
const scanProjectFiles = async (scanOptions) => {
    ...
    step("扫描项目文件...");
    try {
        const vueFiles = getVueFiles(path$5.resolve(projectPath, (options.input || "").replace(/^\//, "")));
...
const generateAutoImportFile = async (importedComponents) => {
    ...
    const autoImportPath = path$5.resolve(projectPath, options.output);
    let fileContent = "";
    if (options.resolvers === "element-ui") {
