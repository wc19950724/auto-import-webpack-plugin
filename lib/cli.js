...
var argsTips = (key) => {
  ...
  switch (key) {
    ...
    case "-e":
      ...
      break;
    case "-o":
      tip = "generator file path".padEnd(PADDING) + `default: '${optionsDefault.output}'`;
      break;
    case "-r":
      tip = "library name now".padEnd(PADDING) + `only: '${optionsDefault.resolvers}'`;
      break;
    case "-i":
      tip = "ignore config path".padEnd(PADDING) + `default: '${optionsDefault.ignorePath}'`;
      break;
    case "-l":
      tip = "log level".padEnd(PADDING) + `default: '${optionsDefault.logLevel}'`;
      break;
    default:
      break;
  }
...
var versionHandler = () => {
  const pkgPath = import_node_path.default.resolve(import_node_path.default.dirname(__dirname), "..", "package.json");
...
var setOptions = async (params) => {
  ...
  const projectIgnorePath = (0, import_node_path2.resolve)(projectPath, options.ignorePath);
  if ((0, import_node_fs2.existsSync)(projectIgnorePath)) {
    const ignoreContext = (0, import_node_fs2.readFileSync)(projectIgnorePath).toString().replace(/\/$/gm, "");
    ignoreFile.add(ignoreContext);
  }
};
var getOptions = () => options;
var getEntryPath = () => {
  return (0, import_node_path2.resolve)(process.cwd(), options.entry);
};
var getOutputPath = () => {
  return (0, import_node_path2.resolve)(projectPath, options.output);
};
var PADDING = 20;
...
var getImportedComponents = () => {
  ...
  if ((0, import_node_fs3.existsSync)(outputPath)) {
    const outputContent = (0, import_node_fs3.readFileSync)(outputPath, "utf8");
...
var getVueFiles = (directory) => {
  const files = (0, import_node_fs3.readdirSync)(directory, { withFileTypes: true });
  const vueFiles = [];
  files.forEach((file) => {
    if (ignoreFile.ignores(file.name))
      return;
    const filePath = (0, import_node_path3.resolve)(directory, file.name);
...
var scanComponents = (file) => {
  ...
  const fileContent = (0, import_node_fs4.readFileSync)(file, "utf-8");
...
var generateAutoImportFile = async (vueComponents) => {
  ...
  if (import_node_fs5.default.existsSync(outputPath)) {
    import_node_fs5.default.unlinkSync(outputPath);
  } else {
    const targetDir = import_node_path4.default.dirname(outputPath);
    if (targetDir !== projectPath) {
      import_node_fs5.default.mkdirSync(targetDir, { recursive: true });
    }
  }
  import_node_fs5.default.writeFileSync(outputPath, fileContent);
