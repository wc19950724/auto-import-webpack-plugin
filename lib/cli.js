...
var scanProjectFiles = async () => {
  ...
  const vueComponents = /* @__PURE__ */ new Set();
  if (options3.resolvers === "element-ui") {
...
var generateAutoImportFile = async (vueComponents) => {
  const options3 = getOptions();
  step(`generating ${options3.output}...`);
  const outputPath = getOutputPath();
  let fileContent = "";
  if (options3.resolvers === "element-ui") {
  ...
  if ((0, import_node_fs4.existsSync)(outputPath)) {
    (0, import_node_fs4.unlinkSync)(outputPath);
  } else {
    const targetDir = (0, import_node_path3.dirname)(outputPath);
    if (targetDir !== projectPath) {
      (0, import_node_fs4.mkdirSync)(targetDir, { recursive: true });
    }
  }
  (0, import_node_fs4.writeFileSync)(outputPath, fileContent);
  step(`${options3.output} genarated!`);
};
...
// src/cli/index.ts
var pkgPath = (0, import_node_path4.resolve)((0, import_node_path4.dirname)(__dirname), "..", "package.json");
var pkg = JSON.parse((0, import_node_fs5.readFileSync)(pkgPath, "utf-8"));
program.name(pkg.name).version(pkg.version).description(pkg.description);
program.option("-e, --entry <name>", "entry path").option("-o, --output <name>", "output file").option("-r, --resolvers <value>", "library name").option("-i, --ignore-path <name>", "entry ignore files config").option("-l, --log-level <value>", "log level").parse(process.argv);
var options2 = program.opts();
main_default(options2).catch((err) => {
  logger_default.error(err.stack);
}).finally(() => {
  process.exit(0);
});
