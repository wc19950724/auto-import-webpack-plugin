...
var writeConfig = (configPath) => __async(void 0, null, function* () {
  var _a;
  const options2 = {};
  ...
  if (!import_node_fs.default.existsSync(fullPath)) {
    ...
    const result = yield lint.lintText(writeData);
    writeData = ((_a = result == null ? void 0 : result[0]) == null ? void 0 : _a.output) || writeData;
...
var formatArgs = (args) => __async(void 0, null, function* () {
  const options2 = {};
  for (const key in args) {
    if (key === "--config") {
      const configPath = args[key] || argsOptionsDefault.config;
      Object.assign(options2, yield writeConfig(configPath));
    }
  }
...
var main = (params) => __async(void 0, null, function* () {
  yield setOptions(params);
...
var cli = () => __async(exports, null, function* () {
  ...
  if (args["--help"]) {
    ...
    const options2 = yield formatArgs(args);
    yield main_default(options2);
  }
});
cli().catch((err) => {
  var _a;
  logger_exports.error((_a = err.stack) != null ? _a : `${err.name}: ${err.message}`);
}).finally(() => {
  process.exit(0);
});
