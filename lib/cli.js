...
let options$1 = optionsDefault;
const importedComponents = new Set();
// 项目根路径
const projectPath = process.cwd(); // 替换为你的项目路径
const ignoreFile = ignore$1().add("node_modules");
const setOptions = async (params) => {
    options$1 = Object.assign({}, options$1, params);
    const projectIgnorePath = node_path.resolve(projectPath, options$1.ignorePath);
...
const scanProjectFiles = async () => {
    ...
    const vueFiles = getVueFiles(node_path.resolve(projectPath, (options.input || "").replace(/^\//, "")));
...
const generateAutoImportFile = async (importedComponents) => {
    ...
    const autoImportPath = node_path.resolve(projectPath, options.output);
    let fileContent = "";
    if (options.resolvers === "element-ui") {
    ...
    if (!ignoreFile.ignores(options.output)) {
        try {
            ...
            const [result] = await eslint$1.lintText(fileContent);
            if (result.output) {
    ...
    if (node_fs.existsSync(autoImportPath)) {
        ...
        // 确保目标目录存在
        const targetDir = node_path.dirname(autoImportPath);
...
const pkgPath = node_path.resolve(__dirname, "..", "package.json");
...
program
    ...
    .option("-r, --resolvers <value>", "components library: element-ui", resolvers)
    .option("-n, --ignore-path <name>", "ignore files config", ignorePath)
    .option("-l, --log-level <value>", "log level: error | wran | info | none | true | false", logLevel)
    .parse(process.argv);
